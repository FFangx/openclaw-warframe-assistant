...
- 愿望命中通知不再依赖按 QQ 会话建立的校准 cron：低频 REST 校准改由 Gateway 插件进程内的定时器承担（默认仍为 10 分钟到期语义，可用 `plugins.config['warframe-fast-commands'].wishlist.calibrationIntervalMs` 调整），实时命中、断线恢复/保护扫描与低频校准三条产出链统一产出**同一个单一 `rich` part**，因此命中卡的「Markdown 图片＋正文＋按钮」结构不再随发现来源变化——此前由 cron 产出的命中只能发媒体与纯文字，图片、文案被拆成两条且没有按钮。恢复/保护扫描与低频校准共用同一执行槽，避免重复请求；升级时按声明前缀幂等清理旧 `warframe-assistant:wishlist:qq:*` 任务。
- 新增愿望卡总开关 `愿望卡 开/关/状态`（别名 `愿望卡片`、`愿望通知卡`，仅 QQ 私聊，默认开启）：关闭后命中、跟踪与状态卡改为先发「文本＋按钮」气泡、再补一张图片，按钮始终可用且图片失败不会重发已成功的那条。偏好只保存账号域与玩家标识的 SHA-256 摘要；读取失败或无可信发送者时按兼容拆分处理。
- 愿望单 CLI 的 `deliver` 子命令改为「校准并原子入队」，不再自行投递（保留为手动/排查入口）；`createSubscriptionsMailer` 明确拒绝 Gateway 才有的 `rich` 载荷（返回 `rich_requires_gateway`），避免载荷 JSON 被当普通文字发给用户。

- QQ 私聊愿望单升级为可操作的持续跟踪卡：每项只命中符合条件的当前最低价，图片、文案与联系卖家/查询当前价格/已购/改价/暂停按钮合并在一条消息；汇总卡可按商品进入管理面板。命中后按前 5 分钟 10 秒、5～30 分钟 30 秒、30～60 分钟 2 分钟的节拍跟踪目标订单，首次缺失 2 秒后精确复核，最长 1 小时；新低价会切换跟踪，同单改价不重置，接口限流或超时不会误报撤下。已购和取消改为一次执行并提供 5 分钟、状态版本保护的撤销。短编号保留为后端和兼容降级标识；自然语言与按钮走同一用例，歧义不默认第一项。愿望单现仅接受 QQ 私聊，历史群聊愿望保留但冻结。
- QQ 私聊中的 `wm` 行情卡现在追加原生操作按钮：默认正文仍给最低价 1 号卖家的 `/w` 模板，按钮按实际挂单数只列 2～5 号卖家，点击后仅把对应私聊模板填入输入框供复制；“走势 物品名”按钮自动查询并返回近 90 天每日成交中位价走势图。群聊不追加这些私聊按钮，按钮不会自动发送游戏聊天或执行交易。
- 完成 R15 第五片统一账号 delta 账本：`AccountSnapshot v1` 先压缩为仅含差分必需字段的嵌套白名单基线，再由助手自有的版本化状态文件生成一次、有界持久化；`drops` 与 `weekly` 从同一事件 ID 流使用独立游标，确认失败会重放且不会互相吞事件。账本采用原子写入、同进程串行、跨进程陈旧锁回收及条数/体积/保留期三重上限，裁剪会显式形成断档；损坏、超前版本或结构漂移只降级不覆盖。掉落通知保持“先入统一 Outbox、再确认游标、最后投递”的恢复顺序，周常 delta 仅作触发与脱敏审计，核销事实继续完全来自当前 `AccountView` 与公开世界状态；不读取 AlecaFrame `deltas.dat`，不保存原始信封、账号标识、令牌或无关字段。
- 增加 R15 第四片版本化 `AccountView v1`：53 项白名单快照字段统一映射为账号、库存、装备、紫卡、集团、周常和商店购买七个语义命名空间；账号概览、库存估值、掉落、紫卡、赏金、轮换、奸商、商店、订阅和周常模块不再直接依赖 AlecaFrame 原始字段名。语义视图兼容现有快照与纯函数测试输入，保留原有数量、时间、核销和展示口径，并通过源码合同阻止业务层重新引入原始顶层、条目或嵌套字段。
- 增加 R15 第二、三片版本化 `AccountSnapshot v1`：AlecaFrame `lastData.dat` 的旧/新信封统一经过 53 项顶层字段白名单及逐字段嵌套投影，原始信封、未消费字段、令牌、账号标识、实例 oid、宠物详情和未来新增键不再向业务层透传；所有输出容器均为新对象，并以加载期门禁保证每个顶层字段都有显式投影。每个保留字段附带来源、同步时间及依据、周期归属、边界、可信度与新鲜度。`alecaframe` 与掉落监测改用同一适配器，现有账号、库存、周常、商店和掉落输出保持兼容。新增纯函数统一 delta，覆盖库存数量及当前周常消费字段，事件身份/顺序稳定、数量有界、首次只建基线，列表只保留计数和稳定摘要而不携带原始内容；当前不读取真实账号快照、不使用 `deltas.dat`、不落盘 delta，也不改变核销或通知状态。
- 周常自动核销为全部确认项增加脱敏周期证据，记录本周起止、快照时间、来源和依据类别，不保存原始账号字段或任务实例 ID；合同测试补齐此前缺少的普通/钢铁回廊、衰退室、普通/钢铁沉沦之地、卡尔，以及两类科研同周同为 34 分的周期核销路径。`账号周常` 同步扩为完整 11 项证据面板，分别显示两类科研分数并标明需要公开世界状态二次对账的项目。
- 增加 R18 第一段模型上下文安全合同：`before_prompt_build` 的动态查询门禁与短命令指代上下文改由可独立测试的纯合成边界生成，模型可见载荷仅保留必要白名单字段，并分别限制桥接 JSON 与总上下文字节数；异常超限时完整丢弃桥接载荷并要求重新查询，不输出半截 JSON。裂缝等带明确业务过期时间的短命令上下文到期后会移除旧摘要、标记为陈旧，只保留实体指代锚点；敏感身份、目标、凭据、原始快照和完整工具结果均由确定性合同阻止进入模型上下文。常规 CI 不调用实时模型或网络。
...
- 1999 日历增益在官方仅提供内部名、效果仍为 `[PH]` 时，不再无限停留在“上游尚未提供中文说明”：后台可先记录有来源的中文名称暂译，卡片明确标注效果待确认；每日扫描持续追查，取得有据完整中文译文后提升替换。修复学习词典在长驻进程中只读一次、更新后卡片仍显示旧文案的问题。
- 钢铁回廊灵化武器的连写内部名（如 `CeramicDagger`）现在能匹配官方带空格英文名及中文词典，并为当前轮换补齐经官方词典核对的静态兜底，不再显示“名称待词典同步”。

- 修复愿望命中卡超出五按钮后，QQ 实机静默丢弃整组按钮并把图片与文字拆开的回归。命中卡现在严格保持查询当前价格、已购、改价、暂停、取消愿望五个按钮；完整 `/w` 已在命中文案中，不再重复提供联系卖家。一体图片载荷失败时只降级为单条“文本＋按钮”，不再发送假装一体的分离媒体消息。
- 修复 1999 日历增益 AI 回退的发现时序：每日后台维护现在会先主动扫描公开世界状态中的整季日历，把未知增益提前放入查证 inbox，不再等用户先打开一次“周常”卡后才发现，避免任务刚跑完时出现最长近 24 小时的中文占位窗口。
