...
- 为 `裂缝` 与 `开遗物` 增加 R12 第一段结构化命令边界：注册表匹配后只调用一次现有参数解析器，并以严格白名单的 `CommandRequest {commandId,args,source,privacyScope}` 贯通 QQ 快捷入口、模型工具、调度 fallback 与脚本子进程。请求拒绝身份、原文、快照等额外字段及未知嵌套字段，限制来源、版本与 8 KiB 体积；执行端收到请求后不得回退重解析中文。两条链同时输出带来源/范围/新鲜度的证据引用和统一 Decision，覆盖入口等价、参数冲突、陈旧证据、缺价及个人权限先行拒绝；其余命令和外部 `operation + query` 协议不变。
- R12 第二片：`裂缝` 与 `开遗物` 的用户可见卡片和文字展示收口到统一 Decision——普通/钢铁分区、候选顺序、结论等级、缺价与陈旧证据语义全部由同一 Decision 驱动（卡片与文字同源），执行结果里与 Decision 冲突的旧字段不再影响展示；候选投影只携带渲染所需、非业务判断的展示事实（中文标签、时间戳、推荐摘要），`建议获取`候选并入 `candidates`（kind='acquire'），`开遗物` 失败回显改由结构化 understanding 生成。展示细节：`开遗物` 文字在估值为空时改为诚实显示「白金估值暂缺 / —」而不再直出 null；`裂缝` 文字在世界状态为缓存快照时标注「来源：世界状态（缓存快照）」，推荐卡片仅按 `personalized` 展示库存提示。新增展示合同测试：修改 Decision 一致改变卡片与文字、旧 data 冲突不影响展示、缺价/陈旧证据/无结果错误/个人权限降级语义与入口协议、隐私字段合同不回退。
- 为 QQ 主硬拦截入口的精确命令 `裂缝 九重天` 增加 R17 第一条请求级脱敏 trace：用同一 `traceId` 串联收到、路由、授权、世界状态/缓存、决策、卡片渲染和 QQ 接受结果。事件只保留 12 个白名单字段与内容哈希，按字节、trace 数和阶段数三重限界；损坏或写入失败不阻断查询。其他命令、备用入口和模型工具暂不接入，待稳定样本验证后再决定是否扩展。

### 修复

- 修复警报/入侵中的非交易奖励只按显示文字翻译、导致 `Umbra Forma Blueprint` 长期落为“未收录奖励”的问题：世界状态规范化现在保留原始 `ItemType`，显示链按物品路径身份反查本机官方简中语言包，并正确保留官方名称 `Umbra Forma`。每日 AI 回填同时要求逐字使用当前 inbox 键并提供 Market/灰机wiki 依据链接，允许官方保留的拉丁专名，拒绝 `umbra`→`umba` 这类错键和无依据写入。
- 1999 日历增益新增受约束的 AI 暂译兜底：可靠简中全链查无但存在完整有据英文名称、效果和链接时，保存英文原文与证据指纹并生成中文；英文依据仅限 DE 官方站点或 WFCD `warframe-worldstate-data` 的精确仓库路径。整项暂译强制显示“（暂译）”，已有可靠中文名而仅由 AI 补效果时显示“（效果暂译）”。可靠静态/社区译名出现后自动取代，资料不足时继续待查而不凭路径猜译；修复 `RefundBulletOnStatusProc / Free Shot` 因世界状态只返回内部路径而长期显示占位的问题。
- 修复周一重置后上游仍短暂返回上周轮换时，周常订阅把旧内容包装成新周卡并提前核销提醒的问题。主动周报现在必须确认执刑官、回廊和两套科研的窗口都已覆盖当前 UTC 周；未完成上游跨周时不渲染、不入 Outbox、不写 `seen`，而是每分钟静默重试。
- 修复已授权用户的`裂缝`库存增强把“兼容遗物存在但部分白金成交统计暂缺”误报成“未匹配库存遗物”：任务先行推荐现在先按实际库存匹配，可靠白金候选仍优先；该纪元全部缺价时按杜卡德期望临时兜底，并明确显示`白金估值暂缺`，不输出建立在残缺白金数据上的精炼建议。成交统计批处理同时保留脱敏失败类别与覆盖计数，新增部分失败、合成快照和`裂缝 九重天`真实命令链回归测试。
- 修复 `开遗物` 成功生成推荐卡后，追问口径文本引用未导入筛选常量而报 `FISSURE_SCOPES is not defined` 的运行时回归。追问文本现由推荐结果模块统一、安全生成；新增合成 AlecaFrame 快照到推荐、卡片与追问文本的纵向执行测试，并在 CI 增加 JavaScript `no-undef` 门禁，防止同类“语法通过、运行时才失败”的未定义符号再次进入主线。
