...
- 新增 Warframe.Market 愿望单：`愿望 商品 价格`（一次最多 5 项）建立愿望价，`愿望单`查看汇总，使用四位短编号执行`改价/暂停/继续/已购/取消`。Gateway 共用一条公开 Market WebSocket 秒级监听新卖单，每个 QQ 会话以 10 分钟 item-top 任务低频校准；新建、改价和恢复后立即检查当前卖单，已有合价订单时在设置反馈后直接补发标准 `wm` 市场行情卡并完成去重，不再延迟到校准任务。提供设置反馈、汇总、命中三类 2x 高清卡与可复制的实际 `/w` 文本；命中不自动交易、不自动聊天、不自动核销。
