...
- 增加 R15 第二片版本化 `AccountSnapshot v1`：AlecaFrame `lastData.dat` 的旧/新信封统一经过顶层字段白名单，原始信封、未消费字段、令牌和账号标识不再向业务层透传；每个保留字段附带来源、同步时间及依据、周期归属、边界、可信度与新鲜度。`alecaframe` 与掉落监测改用同一适配器，现有账号、库存、周常、商店和掉落输出保持兼容。新增纯函数统一 delta，覆盖库存数量及当前周常消费字段，事件身份/顺序稳定、数量有界、首次只建基线，列表只保留计数和稳定摘要而不携带原始内容；本片不读取真实账号快照、不使用 `deltas.dat`、不落盘 delta，也不改变核销或通知状态。
