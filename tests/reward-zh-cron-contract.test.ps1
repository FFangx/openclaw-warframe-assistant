...
if ($null -ne $job) {
  ...
  Assert-Contains 'message: calendar insufficient evidence stays pending' $message '若连官方路径也无法证实，则保留待查，不得 dismiss'
  Assert-Contains 'message: name-only provisional is allowed for official paths' $message '可先根据英文内部名保守生成纯中文名称暂译'
  Assert-Contains 'message: name-only provisional does not invent effects' $message '不得编造百分比、触发条件或效果'
  Assert-Contains 'message: provisional remains in daily verification' $message '已存在 AI 暂译的条目仍须逐日核对官方简中'
