...
// 物品图预热：把 wm 全目录 thumb 一次性下载进磁盘缓存（item-images/）。
...
const wanted = items
  .map((item) => item?.i18n?.en?.thumb)
  .filter(Boolean)
  .map((thumb) => ({ thumb, file: thumb.split('/').at(-1).replace(/[^\w.-]/gu, '_').slice(-80) }))
...
await Promise.all(Array.from({ length: CONCURRENCY }, async () => {
  while (index < wanted.length) {
    const { thumb, file } = wanted[index]; index += 1;
    try {
      const response = await fetch(`https://warframe.market/static/assets/${thumb}`, { signal: AbortSignal.timeout(20_000) });
