...
const COMMAND_REGISTRY = freeze([
  ...
  command({
    ...
    helpTitle: '愿望单',
    helpSummary: '私聊按钮管理；命中最低价后跟踪卖单最长 1 小时',
    helpExamples: [
      { command: '愿望 商品 价格', description: '建立目标价；已有合价单时立即返回行情' },
      { command: '愿望单', description: '查看全部愿望；可直接点商品按钮管理' },
      { command: '改价/暂停/继续 <物品或短编号>', description: '调整目标价或暂时停用、恢复盯价' },
      { command: '已购/取消 <物品或短编号>', description: '一键结束愿望；5 分钟内可撤销' },
    ],
    nextActions: [],
    matchers: [
      ...
      { routes: ['wishlist', 'shortcut-gate'], kind: 'regex', pattern: '^(?:愿望\\s*)?(?:撤销已购|撤销取消|已购|买到|改价|暂停|继续|恢复|取消)(?:\\s+|$).+', flags: 'u' },
    ],
  }),
]);
