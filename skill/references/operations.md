...
- `--card-dir` 存在时发现新情报生成深色提醒卡；`monitor` 输出 `MEDIA:` 供兼容调用，生产订阅 cron 使用 `deliver` 子命令逐图调用 QQ outbound 并关闭 runner announce；超长周报在主动推送前按原像素切为 3 段（160px 边缘重叠，单段最高 4000px），避免 QQ 按整张纵横比激进缩小；渲染/切片失败分别退文字/原长图
...
- `订阅 周常`：创建后首轮即推本周清单；之后每周一 00:00 UTC 刷新后推两张卡（单张原始 PNG 周报大卡+本周好货卡，好货卡挂了降级只发周报）；
  优先级最低不挤占其他提醒；周报通过 QQ `/files` 的 `srv_send_msg=true` 一步直发（不再追加会压缩长图的 `msg_type=7`），两张卡全部成功后才核销本周去重标记，失败留待下一分钟重试
