...
- 1999 日历增益：DE 官方语言键与公开导出常不含增益名。解析优先级（weekly.mjs `calendarUpgradeEntry`）：① 灰机wiki 用户核验静态路径表 → ② 社区维护状态中文表 → ③ 学习词典 → ④ 静态题名表 → 诚实占位。未知路径进入 `.cache/warframe-data/calendar-upgrade-inbox.json`；每日任务先查可靠简中，有完整名称和效果才写普通学习条目。可靠简中查无时，仅接受 `warframe.com`、`forums.warframe.com` 或 `wiki.warframe.com` 的完整官方英文依据来生成 AI 暂译，使用 `--provisional true --english-name --english-desc --evidence-url` 落库；渲染强制标注“（暂译）”，并保存英文原文、URL 与 SHA-256 指纹。官方英文资料不足则保持待查，不根据内部路径猜译。静态/社区来源始终优先，后来取得的可靠简中可替换暂译
