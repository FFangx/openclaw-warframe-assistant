...
- 1999 日历增益：DE 官方语言键与公开导出常不含增益名。解析优先级（weekly.mjs `calendarUpgradeEntry`）：① 灰机wiki 用户核验静态路径表 → ② 社区维护状态中文表 → ③ 学习词典 → ④ 静态题名表 → 诚实占位。未知路径进入 `.cache/warframe-data/calendar-upgrade-inbox.json`；每日任务先查可靠简中，有完整名称和效果才写普通学习条目。可靠简中查无时，英文依据仅接受 `warframe.com`、`forums.warframe.com`、`wiki.warframe.com`，或按完整内部路径精确命中的 WFCD `warframe-worldstate-data/data/languages.json`；WFCD 仅放行 `github.com/WFCD/warframe-worldstate-data/` 与对应 `raw.githubusercontent.com` 精确路径，不信任任意 GitHub 内容。英文名称、完整效果和链接齐全才可用 `--provisional true --english-name --english-desc --evidence-url` 落库；整项暂译显示“（暂译）”，可靠名称只缺说明时显示“（效果暂译）”，并保存英文原文、URL 与 SHA-256 指纹。有据英文资料不足则保持待查，不根据内部路径猜译。静态/社区来源始终优先，后来取得的可靠简中可替换暂译
