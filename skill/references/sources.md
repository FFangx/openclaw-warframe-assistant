...
- 世界状态：PC 首选 `GET https://api.warframe.com/cdn/worldState.php` 并本地规范化；官方故障时 `GET https://api.warframestat.us/{platform}` 作为全量备用；browse.wf Oracle `GET https://oracle.browse.wf/worldState.min.json`（公开开源 browse.wf 实时客户端端点）是**部分镜像**——实测只有 Events/Goals/Alerts/Sorties/LiteSorties/ActiveMissions/VoidTraders/VoidStorms/DailyDeals/Conquests/Tmp 等裁剪键，无 Invasions/SyndicateMissions/SeasonInfo/EndlessXpSchedule/KnownCalendarSeasons，因此只有官方失败且 warframestat 全量成功时作**裂缝字段级叠加**（Oracle 专属合同只要求 ActiveMissions/VoidStorms——**实况端点无顶层 Time**，HTTP 响应只有 Date/Last-Modified/ETag/Cache-Control: public,max-age=10，Date 是服务器响应时间不是内容时间，上游内容时间以 HTTP Last-Modified 为准；仅规范化裂缝/虚空风暴）；叠加结果不写入可靠缓存，绝不能单独作为完整状态返回；`GET /{platform}` 也用于非 PC，PC 侧兼作非阻塞交叉验证；科研轮换可用 warframestat 子端点 `GET /{platform}/archimedeas` 做字段级补取
- 科研词缀中文：`oracle.browse.wf/dicts/en.json` × `zh.json`（227 键，其中 `/Lotus/Language/Conquest/` 200 键）。两份索引：英文显示名 → 候选（warframestat 路径用，按说明原文/数字消歧）；语言键尾段（剥 `Condition_/PersonalMod_/MissionVariant_[Lab|Hex]Conquest_` 前缀后）→ 候选（官方备用源只有路径尾段时直查，按 LAB/HEX 前缀消歧重名，如 Reinforcements＝LAB 协调阵线/HEX 支援）。名称与 `_Desc` 官方简中说明同源返回；缓存 24 小时。词典说明中的 `<DT_*>` 等客户端富文本标记在呈现前统一剥离，剩余尖括号/竖线模板或裸引擎 token 触发安全降级；不再按每周单键补漏
...
- 普通参考请求使用 20 秒超时；Market 主查询采用两次 8 秒的总预算内分类重试，PC worldstate 的 warframestat 交叉验证/备用与 browse.wf Oracle 裂缝层各自采用 6 秒快速探测并配合持久健康退避，但不阻塞已验证的 DE 官方结果。搜索仍需区分“无匹配”与网络或 HTTP 失败
...
- PC worldstate 的 warframestat 交叉验证/备用源与 browse.wf Oracle 裂缝层（端点健康键 `worldstate:oracle:pc`）遇 403 首次即打开 15 分钟端点熔断；网络/超时连续失败使用 30 秒起的指数退避（最高 5 分钟）。退避状态写入本地 `endpoint-health.v1.json`，跨短命令进程生效。合同按来源区分：DE 官方原始响应走官方全量合同（8 个关键集合 + Time），Oracle 原始响应走 Oracle 专属合同（ActiveMissions/VoidStorms；Oracle 是部分镜像，永不带 Invasions/SyndicateMissions 等，**也没有顶层 Time**——上游内容时间取 HTTP Last-Modified）；官方规范化结果通过完整字段合同后才写入可靠缓存，Oracle 的 fissures 归一化结果只作字段级叠加、不写可靠缓存，HTTP 200 但结构残缺不得覆盖可靠缓存
- 每个在线 PC 世界状态快照携带来源质量信封 `_envelope`（provider / fetchedAt / 上游时间 / 请求延迟 / 关键集合完整性缺失清单 / 内容哈希，上游时间=官方顶层 `Time`、Oracle=已验证的 HTTP Last-Modified）与按顶层字段的 `_fieldProviders`，doctor 与巡检可据此区分字段来自哪个提供者、多新鲜、是否完整。官方失败 + warframestat 成功的叠加快照：`_fieldProviders.fissures='oracle.browse.wf'`、其余字段 `'api.warframestat.us'`，另附 `_oracleEnvelope`（`partial:true`，scope=ActiveMissions/VoidStorms，`upstreamTime`=HTTP Last-Modified）与 `_composite` 元数据，叠加对象不写入可靠缓存、不冒充单一全量来源；Oracle 裂缝层叠加前还须通过上游年龄门禁（HTTP Last-Modified 缺失/无效/超过 15 分钟一律拒绝——不能用响应 `Date` 或本机 fetchedAt 冒充上游内容时间；官方主源即权威，不设该门禁）与裂缝事件 ID 连续性核对（与最近 10 分钟内写入的可靠规范化快照零交集即判定分歧并拒绝叠加）。Oracle 内层缓存 v3 存「原始载荷 + 已验证 Last-Modified/ETag/Cache-Control」包，缓存命中保留原始 Last-Modified、不因读取而刷新年龄。Oracle 是部分镜像：全在线源失败时绝不以 Oracle 单独充当完整状态，维持「可靠缓存 + 真实 stale/cachedAt」回退。多处社区宿主可能均派生自 DE 同一上游：属于托管可用性多宿主，不是独立事实源
...
- 1999 日历增益：DE 官方语言键与公开导出常不含增益名。解析优先级（weekly.mjs `calendarUpgradeEntry`）：① 灰机wiki 用户核验静态路径表 → ② 社区维护状态中文表 → ③ 学习词典 → ④ 静态题名表 → 诚实占位。未知路径进入 `.cache/warframe-data/calendar-upgrade-inbox.json`；每日任务先用公开世界状态主动扫描当前整季日历，再查可靠简中，因此不再依赖用户先打开周常卡才发现缺口。有完整名称和效果才写普通学习条目。可靠简中查无时，英文依据仅接受 `warframe.com`、`forums.warframe.com`、`wiki.warframe.com`，或按完整内部路径精确命中的 WFCD `warframe-worldstate-data/data/languages.json`；WFCD 仅放行 `github.com/WFCD/warframe-worldstate-data/` 与对应 `raw.githubusercontent.com` 精确路径，不信任任意 GitHub 内容。英文名称、完整效果和链接齐全才可用 `--provisional true --english-name --english-desc --evidence-url` 落库；整项暂译显示“（暂译）”，可靠名称只缺说明时显示“（效果暂译）”，并保存英文原文、URL 与 SHA-256 指纹。有据英文资料不足则保持待查，不根据内部路径猜译。静态/社区来源始终优先，后来取得的可靠简中可替换暂译
...
- 奸商购物/开遗物商品模式的「市场路线决策价」例外：以当前卖单 `top` 的稳健低值为准（最低价与次低价差距 <70% 且低于今日成交中位 <60% 时按钓鱼/抢跑单剔除改用次低价），无有效卖单才回退上一条的今日/90 天成交中位口径；今日/90 天中位始终作为对照展示。Baro 到访期低价挂单立即反映，但钓鱼单不会砸穿参考价
- 部件级「已入库」判定（杜卡德默认保留）：从本机 AlecaFrame `Relics.json`（与遗物反查同一张表）建立「部件→遗物」索引，一个部件出现在多条遗物时按「全部遗物均已入库」才算已入库；索引构建失败或查无该部件时状态为未知，按未入库（可刷）处理，不误拦新物品
...
- `获取 <Prime部件>`复用反向搜索结果，并以 WFCD 静态遗物掉点作为常驻/条件/悬赏轮换来源；当前开放世界赏金必须再由实时 worldstate 核对，核对失败时静态悬赏不得标成当前开放
