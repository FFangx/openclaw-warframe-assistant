...
- v2 当前没有价格历史端点；成交统计只读使用 v1 `GET /v1/items/{slug}/statistics` 的 `statistics_closed`。`48hours` 为小时桶、`90days` 为日桶；奸商推荐按等级过滤后优先取今日至少 3 笔成交的加权中位，否则回退 90 天加权中位，并以 90 天总成交量/90 显示日均成交量。不得把在线挂单伪装成成交价
