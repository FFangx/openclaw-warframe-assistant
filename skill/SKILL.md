...
- **个人（仅主人 QQ 私聊）**：`杜卡德 [目标|清仓] [保留N|保留N套]` ｜ `裂缝推荐 [白金|杜卡德] [速刷|舒适|收益] [单人]` ｜ `精炼推荐 [单人|杜卡德]` ｜ `奸商推荐` ｜ `轮换日历` ｜ `我的紫卡`/`紫卡 <序号|武器>` ｜ `商店 [序号|商人]` ｜ `本周好货` ｜ `我的账号` ｜ `我的库存 [分类|物品]` ｜ `我的遗物/赋能 <名>` ｜ `账号周常`
...
- **自然语言五级决策（本轮无插件注入数据时，顺序不可乱）**：
  1. **模板路由（首选）**：意图能对应模板（目录见 `dispatch.mjs list`；概览：问价→`wm X [等级]`、哪里出→`遗物 X`、裂缝/仲裁/警报/入侵/活动/突击/钢铁侵袭/虚空商人→同名命令、赏金→`赏金 [地点|物品]`、周常→`周常`/`完成 N`、兑换 Prime 部件→`杜卡德 [目标|清仓]`、开什么遗物→`裂缝推荐 [白金|杜卡德] [速刷|舒适|收益] [单人]`、值得精炼→`精炼推荐`、奸商买什么→`奸商推荐`、商店/好货→`商店`/`本周好货`、哪里买→`哪里买 X`、库存→`我的库存 X`、功能咨询→`帮助`）。归一成规范命令后跑 `dispatch.mjs run "<命令>"`；周常/订阅类附 `--target --owner`；**个人数据命令只有确认当前是主人私聊才传 `--personal-allowed true`，否则不传**。
