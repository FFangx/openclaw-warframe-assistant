...
- **个人（仅用户本人 QQ 私聊）**：`杜卡德 [目标|清仓] [保留N|保留N套]` ｜ `开遗物 [商品名|对标 商品名|未入库|已入库] [白金|杜卡德] [钢铁|九重天] [速刷|舒适|收益] [单人]` ｜ `精炼推荐 [单人|杜卡德]` ｜ `奸商推荐` ｜ `轮换日历` ｜ `我的紫卡`/`紫卡 <序号|武器>` ｜ `商店 [序号|商人]` ｜ `本周好货` ｜ `我的账号` ｜ `我的库存 [分类|物品]` ｜ `我的遗物/赋能 <名>` ｜ `账号周常`。`钢铁`只匹配钢铁裂缝；`九重天`只匹配虚空风暴裂缝（硬筛选，不是偏好，也绝不进入奸商商品模式）；`开遗物 商品名`直接进入当前奸商商品模式，旧写法`开遗物 杜卡德 商品名`继续兼容，`开遗物 对标 商品名`是更明确的新写法；同机存在配套 WFInfo 时会同步游戏内开奖策略。
- **周常**：`周常`（=`周报`）一图流 11 项＋快照自动打卡 ｜ `完成/撤销/跳过/取消跳过 <编号|名称>` ｜ `清空周常`
- **愿望单（仅 QQ 私聊，公共市场只读）**：`愿望 <商品> <价格>`（可用 `；` 一次设置多项，支持 `等级N/满级`）｜`愿望单` 汇总｜`已购/改价/暂停/继续/取消 <商品或备用短编号>`｜`愿望卡 <开|关|状态>` 控制愿望卡片是否合并成一条（默认开启，关闭则先发文本＋按钮再补图）。命中卡只选择当前最低价，图片、含完整 `/w` 的文案与按钮在同一消息；可点“查询当前价格 / 已购 / 改价 / 暂停 / 取消愿望”。命中后最长跟踪该订单 1 小时；已购和取消可在 5 分钟内撤销。短编号只作歧义与按钮失效时的后备标识，模型不得在同名多等级愿望中自行选第一项。
...
- 自然语言建立、改价、暂停、恢复、已购、取消或查看愿望时，规范成同一愿望命令后调用 `warframe_assistant operation=command`；商品名只匹配一项可直接执行，命中多个等级/状态时必须把工具返回的候选项交给用户选择，禁止默认第一项。
