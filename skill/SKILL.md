...
- **愿望单（仅 QQ 私聊，公共市场只读）**：`愿望 <商品> <价格>`（可用 `；` 一次设置多项，支持 `等级N/满级`）｜`愿望单` 汇总｜`已购/改价/暂停/继续/取消 <商品或备用短编号>`。命中卡只选择当前最低价，图片、文案与按钮在同一消息；可点“联系卖家 / 查询当前价格 / 已购 / 改价 / 暂停”。命中后最长跟踪该订单 1 小时；已购和取消可在 5 分钟内撤销。短编号只作歧义与按钮失效时的后备标识，模型不得在同名多等级愿望中自行选第一项。
