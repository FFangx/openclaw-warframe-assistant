...
> 以及公开仓库中的持续分发状态。代码与文档的授权见根目录 `LICENSE`（MIT）；数据源的归属见 `NOTICE.md`。
> 复核日期：2026-08-29（含仓库清理与当前展示图复核）。本文件不构成法律意见；
...
| 文件 | 内容 | 状态 |
|---|---|---|
| `img/showcase-fissure-speed.png`、`img/showcase-bounty-aya.png`、`img/showcase-open-relic.png`、`img/showcase-ducat-600.png`、`img/showcase-weekly.png`、`img/showcase-my-rivens.png` | README 当前使用的六张本项目卡片展示图，画面含 DE 游戏素材与示例性的本机只读统计；人工复核未见 QQ/OpenID、账号名、凭据、卖家身份或原始快照 | ✅ 保留作功能展示（附条件：只展示必要的游戏内统计，不加入外部身份或凭据）。四张已被替代且不再引用的旧 `card-*.png` 已于 2026-08-29 从仓库删除 |
...
- [x] `img/`：只保留 README 当前引用且已人工复核的六张展示图；四张无引用旧图已删除。
...
- 2026-08-29：仓库已公开，重新核对 README 实际引用与展示内容；删除四张无引用旧卡片截图，只保留六张当前展示图，
  并确认画面不含外部身份、凭据、卖家信息或原始个人快照。
