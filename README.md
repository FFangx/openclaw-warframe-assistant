...
- **杜卡德规划**（需 [AlecaFrame](https://alecaframe.com)）：`杜卡德 600` 自动找白金损失最低的兑换组合；默认按成品拥有状态智能保留——已拥有成品可换最后一件，未拥有或状态未知保留一套；`保留N/保留N套` 可显式覆盖
- **奸商路线比较**：Prime 部件机会成本＋奸商现金，对比 0 级市场价＋准确交易税，告诉你该换还是直接买
- **个人数据**（需 [AlecaFrame](https://alecaframe.com)）：`开遗物`按遗物价值推 TOP8、指定奸商商品的兑杜/卖白金决策、库存估值、掉落监测、紫卡估价、精炼/奸商购物推荐、商店已购对账、本周好货
